/**
 * Topic: Arrays (Part 1)
 * Problem: Pairs and Subarrays in an Array
 * 
 * Description:
 * Demonstrates how to print all possible unique pairs, all possible subarrays,
 * and the sum of all subarrays from a given array.
 * 
 * Intuition:
 * - Pairs: Use two nested loops. Outer loop fixes the first element, inner loop
 * picks the second.
 * - Subarrays: Use three nested loops. Outer loop for start index, middle loop
 * for end index,
 * inner loop to print elements between start and end.
 * 
 * Complexity:
 * - Pairs: O(N^2) Time, O(1) Space. (Total pairs = N(N-1)/2)
 * - Subarrays: O(N^3) Time, O(1) Space. (Total subarrays = N(N+1)/2)
 */
 ...
 * - Similar to subArrays method but keeps a 'sum' variable to accumulate the
 * sum of each subarray.
 * - 'sum' is reinitialized to 0 for each subarray to avoid carrying over
 * values.
 * - Example (for arr = {1, 2}):
 * 1 sum is: 1
 * 12 sum is: 3
